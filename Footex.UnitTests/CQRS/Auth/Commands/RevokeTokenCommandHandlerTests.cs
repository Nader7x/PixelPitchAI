...
public class RevokeTokenCommandHandlerTests
{
    ...
    {
        // Arrange
        var command = _fixture.Build<RevokeTokenCommand>()
            .With(x => x.RefreshToken, "valid-refresh-token")
            .With(x => x.IpAddress, "192.168.1.1")
            .Create();

        _mockTokenService.Setup(x => x.RevokeTokenAsync(command.RefreshToken, command.IpAddress))
        ...
        _mockTokenService.Verify(x => x.RevokeTokenAsync(command.RefreshToken, command.IpAddress), Times.Once);
    ...
    public async Task Handle_InvalidRefreshToken_ShouldStillCallTokenService(string refreshToken)
    {
        ...
        _mockTokenService.Setup(x => x.RevokeTokenAsync(refreshToken, command.IpAddress))
        ...
        _mockTokenService.Verify(x => x.RevokeTokenAsync(refreshToken, command.IpAddress), Times.Once);
    }
    ...
    public async Task Handle_InvalidIpAddress_ShouldStillCallTokenService(string ipAddress)
    {
        ...
        _mockTokenService.Setup(x => x.RevokeTokenAsync(command.RefreshToken, ipAddress))
            .Returns(Task.CompletedTask);
        ...
        _mockTokenService.Verify(x => x.RevokeTokenAsync(command.RefreshToken, ipAddress), Times.Once);
        _mockUnitOfWork.Verify(x => x.SaveChangesAsync(It.IsAny<CancellationToken>()), Times.Once);
    }
