...
namespace Footex.UnitTests.CQRS.Seasons.Commands
{
    public class DeleteSeasonCommandHandlerTests
    {
        ...
        public async Task Handle_WithValidCommand_ReturnsSuccessResponse()
        {
            ...
            _unitOfWorkMock.Setup(x => x.Seasons.GetByIdAsync(command.Id)).ReturnsAsync(season);
            ...
            _unitOfWorkMock
                .Setup(x =>
                    x.TeamSeasons.GetAllAsync(It.IsAny<Expression<Func<TeamSeasons, bool>>>())
                )
                .ReturnsAsync(new List<TeamSeasons>());

            _unitOfWorkMock.Setup(x => x.Seasons.DeleteAsync(season));
            ...
            _unitOfWorkMock.Verify(x => x.Seasons.DeleteAsync(season), Times.Once);
        ...
        public async Task Handle_WithAssociatedMatches_ReturnsErrorResponse()
        {
            ...
            var season = _fixture
                .Build<Season>()
                .With(s => s.Id, command.Id)
                .With(s => s.Matches, new List<Match> { new Match(1, "test") })
                .Without(s => s.SeasonTeams)
                .Create();
            _unitOfWorkMock.Setup(x => x.Seasons.GetByIdAsync(command.Id)).ReturnsAsync(season);
            _unitOfWorkMock
        ...
        public async Task Handle_WithAssociatedTeamStatistics_ReturnsErrorResponse()
        {
            ...
            var season = _fixture
                ...
                .With(s => s.Id, command.Id)
                .Without(s => s.Matches)
                .With(s => s.SeasonTeams, new List<TeamSeasons> { new TeamSeasons() })
                .Create();
            ...
            result.Should().NotBeNull();
            result.Succeeded.Should().BeFalse();
            result.Error.Should().Be("Cannot delete season as it has associated team statistics");
        }
        ...
        public async Task Handle_WhenSaveChangesFails_ReturnsFailureResponse()
        {
            ...
            // Verify that DeleteAsync was called, but SaveChangesAsync was attempted
            _unitOfWorkMock.Verify(x => x.Seasons.DeleteAsync(season), Times.Once);
            _unitOfWorkMock.Verify(
                x => x.SaveChangesAsync(It.IsAny<CancellationToken>()),
                Times.Once
