    ...
    : IRequestHandler<DeleteSeasonCommand, DeleteSeasonCommandResponse>
{
    public async Task<DeleteSeasonCommandResponse> Handle(DeleteSeasonCommand request, CancellationToken cancellationToken)
    {
        try
        {
            ...
            var matches = await unitOfWork.Matches.FindAsync(m => m.HomeTeamSeasonId == request.Id || m.AwayTeamSeasonId == request.Id);
            if (matches != null)
            {
                return new DeleteSeasonCommandResponse
                {
            ...
            var teamStats = await unitOfWork.TeamSeasons.FindAsync(ts => ts.SeasonId == request.Id);
            if (teamStats != null)
            {
                return new DeleteSeasonCommandResponse
                {
