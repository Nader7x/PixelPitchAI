...
{
    private readonly bool _enablePostgresFts = configuration.GetValue("Search:EnablePostgreSQLFullText", true);
    private readonly int _fuzzySearchThreshold = configuration.GetValue("Search:FuzzySearchThreshold", 3);
    ...
    public async Task<SearchResultDto> SearchAllAsync(string query, int page = 1, int pageSize = 10,
    ...
    public async Task<SearchResultDto> SearchWithStrategyAsync(string query,
        SearchStrategy strategy = SearchStrategy.Auto, int page = 1, int pageSize = 10)
    {
        ...
        try
        {
            ...
            var selectedStrategy = strategy == SearchStrategy.Auto ? DetermineOptimalStrategy(query) : strategy;

            logger.LogDebug("Using search strategy: {Strategy} for query: '{Query}'", selectedStrategy, query);
    ...
    public async Task<SearchResultDto> SearchWithFiltersAsync(SearchFiltersDto filters)
    {
        if (filters == null)
        {
            ...
            return new SearchResultDto { TotalResults = 0, Items = [], CurrentPage = 1, PageSize = 10, TotalPages = 0 };
        }
        ...
        try
        {
            ...
            var typesToSearch = filters.EntityTypes != null && filters.EntityTypes.Any()
                ? filters.EntityTypes
            ...
            foreach (var entityType in typesToSearch)
                if (entityType.Equals("Team", StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(filters.Country))
                        teamQuery = teamQuery.Where(t =>
                            t.Country != null && t.Country.Equals(filters.Country, StringComparison.OrdinalIgnoreCase));
                ...
                else if (entityType.Equals("Player", StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    if (lowerQuery != null)
                        playerQuery = playerQuery.Where(p =>
                            (p.FullName != null && p.FullName.ToLowerInvariant().Contains(lowerQuery)) ||
                            (p.KnownName != null && p.KnownName.ToLowerInvariant().Contains(lowerQuery)) ||
                            (p.Position != null &&
                             p.Position.ToLowerInvariant().Contains(lowerQuery)) || // Assuming Player has Position
                            (p.Nationality != null && p.Nationality.ToLowerInvariant().Contains(lowerQuery)));
                    if (!string.IsNullOrWhiteSpace(filters.Country)) // Using general Country for Player Nationality
                        playerQuery = playerQuery.Where(p =>
                            p.Nationality != null &&
                            p.Nationality.Equals(filters.Country, StringComparison.OrdinalIgnoreCase));
                ...
                else if (entityType.Equals("Coach", StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    if (lowerQuery != null)
                        coachQuery = coachQuery.Where(c =>
                            (c.FirstName != null && c.FirstName.ToLowerInvariant().Contains(lowerQuery)) ||
                            (c.LastName != null && c.LastName.ToLowerInvariant().Contains(lowerQuery)) ||
                            (c.Nationality != null && c.Nationality.ToLowerInvariant().Contains(lowerQuery)));
                    if (!string.IsNullOrWhiteSpace(filters.Country)) // Using general Country for Coach Nationality
                        coachQuery = coachQuery.Where(c =>
                            c.Nationality != null &&
                            c.Nationality.Equals(filters.Country, StringComparison.OrdinalIgnoreCase));
                ...
                else if (entityType.Equals("Stadium", StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(filters.Country))
                        stadiumQuery = stadiumQuery.Where(s =>
                            s.Country != null && s.Country.Equals(filters.Country, StringComparison.OrdinalIgnoreCase));
                    ...
                    if (filters.MinCapacity.HasValue)
                        stadiumQuery = stadiumQuery.Where(s => s.Capacity >= filters.MinCapacity.Value);
                    if (filters.MaxCapacity.HasValue)
                        stadiumQuery = stadiumQuery.Where(s => s.Capacity <= filters.MaxCapacity.Value);

                    var stadiumData = await stadiumQuery.Select(s => new
                    {
                        s.Id,
                        s.Name,
                        s.City,
                    ...
                    allFilteredItems.AddRange(stadiumData.Select(s => new SearchItemDto
                    {
                        Id = s.Id.ToString(),
                        Type = "Stadium",
                        Name = s.Name ?? "N/A",
                        Description = $"City: {s.City ?? "N/A"}, Capacity: {s.Capacity}, Country: {s.Country ?? "N/A"}",
                ...
                {
                    var matchQuery = context.Matches.Include(m => m.HomeTeam).Include(m => m.AwayTeam).AsQueryable();
                    if (lowerQuery != null)
                    ...
                    if (filters.FromDate.HasValue)
                        matchQuery = matchQuery.Where(m => m.ScheduledDateTimeUtc >= filters.FromDate.Value);
                    ...
                    allFilteredItems.AddRange(matchData.Select(m => new SearchItemDto
                    {
                        Id = m.Id.ToString(),
                        Type = "Match",
            ...
            if (!string.IsNullOrWhiteSpace(filters.SortBy))
                ...
                try
                {
                    ...
                    {
                        logger.LogWarning("SortBy property '{SortByProperty}' not found on SearchItemDto.",
                            filters.SortBy);
                    }
                }
                catch (Exception ex)
        ...
        {
            logger.LogError(ex, "Error occurred during SearchWithFiltersAsync for filters: {@Filters}", filters);
            return new SearchResultDto
            {
                TotalResults = 0, Items = [], CurrentPage = filters.Page, PageSize = filters.PageSize, TotalPages = 0,
                Error = "An error occurred during the search."
            };
        }
    }

    public async Task<List<SearchSuggestionDto>> GetSearchSuggestionsAsync(string query, int limit = 5)
    {
    ...
    public async Task<List<Team>> SearchTeamsAsync(string query, int limit = 10, bool enableFuzzySearch = false)
    {
        if (string.IsNullOrWhiteSpace(query))
            return [];
    ...
    public async Task<List<Player>> SearchPlayersAsync(string query, int limit = 10, bool enableFuzzySearch = false)
    {
        if (string.IsNullOrWhiteSpace(query))
            return [];
    ...
    public async Task<List<Coach>> SearchCoachesAsync(string query, int limit = 10, bool enableFuzzySearch = false)
    {
        if (string.IsNullOrWhiteSpace(query))
            return [];
    ...
    public async Task<List<Stadium>> SearchStadiumsAsync(string query, int limit = 10, bool enableFuzzySearch = false)
    {
        if (string.IsNullOrWhiteSpace(query))
            return [];
    ...
    public async Task<List<Season>> SearchSeasonsAsync(string query, int limit = 10, bool enableFuzzySearch = false)
    {
        if (string.IsNullOrWhiteSpace(query))
            return [];
    ...
    public async Task<List<Match>> SearchMatchesAsync(string query, int limit = 10, bool enableFuzzySearch = false)
    {
        if (string.IsNullOrWhiteSpace(query))
            return [];
    ...
    public async Task<List<Player>> SearchPlayersWithAdvancedRankingAsync(string query, int limit = 10)
    {
        if (string.IsNullOrWhiteSpace(query))
            return [];
    ...
    public async Task<List<Coach>> SearchCoachesWithAdvancedRankingAsync(string query, int limit = 10)
    {
        if (string.IsNullOrWhiteSpace(query))
            return [];
    ...
    public async Task<List<Stadium>> SearchStadiumsWithAdvancedRankingAsync(string query, int limit = 10)
    {
        if (string.IsNullOrWhiteSpace(query))
            return [];
    ...
    public async Task<SearchResultDto> UnifiedSearchAsync(string query, List<string>? entityTypes = null, int page = 1,
        int pageSize = 10)
    {
        if (string.IsNullOrWhiteSpace(query) || query.Length < 2)
            return new SearchResultDto
        ...
        try
        {
            ...
            if (typesToSearch.Contains("Team"))
                searchTasks.Add(Task.Run(async () =>
                {
                    ...
                    return teams.Select(t => new SearchItemDto
                    {
                        Id = t.Id.ToString(),
                        Type = "Team",
            ...
            if (typesToSearch.Contains("Player"))
                searchTasks.Add(Task.Run(async () =>
                {
                    ...
                    return players.Select(p => new SearchItemDto
                    {
                        Id = p.Id.ToString(),
                        Type = "Player",
            ...
            if (typesToSearch.Contains("Coach"))
                searchTasks.Add(Task.Run(async () =>
                {
                    ...
                    return coaches.Select(c => new SearchItemDto
                    {
                        Id = c.Id.ToString(),
                        Type = "Coach",
            ...
            if (typesToSearch.Contains("Match"))
                searchTasks.Add(Task.Run(async () =>
                {
                    ...
                    return matches.Select(m => new SearchItemDto
                    {
                        Id = m.Id.ToString(),
                        Type = "Match",
                        Name = $"{m.HomeTeam?.Name} vs {m.AwayTeam?.Name}",
                        Description = $"{m.ScheduledDateTimeUtc:MMM dd, yyyy}",
                        Url = $"/matches/{m.Id}", AdditionalData = new Dictionary<string, string?>
                        {
            ...
            if (typesToSearch.Contains("Stadium"))
                searchTasks.Add(Task.Run(async () =>
                {
                    ...
                    return stadiums.Select(s => new SearchItemDto
                    {
                        Id = s.Id.ToString(),
                        Type = "Stadium",
    ...
    {
        return Regex.Replace(query.Trim(), @"[^\w\s]", " ")
            .Replace("'", "''")
            .Trim();
    }
    ...
    private async Task<List<(SearchItemDto Item, float Score)>> SearchTeamsWithAdvancedRankingAsync(
        string normalizedQuery, string phraseQuery, string prefixQuery)
    {
        if (!_enablePostgresFts)
        {
        ...
        var results = await context.Teams
            .FromSqlRaw(@"
                SELECT t.*, 
    ...
    private async Task<List<(SearchItemDto Item, float Score)>> SearchPlayersWithAdvancedRankingAsync(
        string normalizedQuery, string phraseQuery, string prefixQuery)
    {
        if (!_enablePostgresFts)
        {
    ...
    private async Task<List<(SearchItemDto Item, float Score)>> SearchCoachesWithAdvancedRankingAsync(
        string normalizedQuery, string phraseQuery, string prefixQuery)
    {
        if (!_enablePostgresFts)
        {
        ...
        var context = scope.ServiceProvider.GetRequiredService<FootballDbContext>();
        // PostgreSQL full-text search with ranking
        var results = await context.Coaches
            .FromSqlRaw(@"
    ...
    private async Task<List<(SearchItemDto Item, float Score)>> SearchMatchesWithAdvancedRankingAsync(
        string normalizedQuery, string phraseQuery, string prefixQuery)
    {
        using var scope = serviceScopeFactory.CreateScope();
        var context = scope.ServiceProvider.GetRequiredService<FootballDbContext>();
        ...
        var results = await context.Matches
            .FromSqlRaw(@"
                SELECT m.*,
    ...
    private async Task<List<(SearchItemDto Item, float Score)>> SearchStadiumsWithAdvancedRankingAsync(
        string normalizedQuery, string phraseQuery, string prefixQuery)
    {
        using var scope = serviceScopeFactory.CreateScope();
        var context = scope.ServiceProvider.GetRequiredService<FootballDbContext>();
        // Stadium search implementation - basic search since stadiums might not have full-text indexes
        var stadiums = await context.Stadiums
            .Where(s => s.Name!.ToLower().Contains(normalizedQuery.ToLower()) ||
    ...
    private async Task<List<SearchItemDto>> SearchTeamsWithFuzzyAsync(string searchTerm)
    {
        ...
        var fuzzyMatches = teams
            .Where(t => CalculateLevenshteinDistance(searchTerm, t.Name!.ToLower()) <= _fuzzySearchThreshold)
    ...
    private async Task<List<SearchItemDto>> SearchPlayersWithFuzzyAsync(string searchTerm)
    {
        ...
        var fuzzyMatches = players
            .Where(p => CalculateLevenshteinDistance(searchTerm, p.FullName!.ToLower()) <= _fuzzySearchThreshold ||
                        (p.KnownName != null && CalculateLevenshteinDistance(searchTerm, p.KnownName.ToLower()) <=
    ...
    private async Task<List<SearchItemDto>> SearchCoachesWithFuzzyAsync(string searchTerm)
    {
        ...
        var fuzzyMatches = coaches
            .Where(c => (c.FirstName != null &&
                         CalculateLevenshteinDistance(searchTerm, c.FirstName.ToLower()) <= _fuzzySearchThreshold) ||
                        (c.LastName != null && CalculateLevenshteinDistance(searchTerm, c.LastName.ToLower()) <=
                            _fuzzySearchThreshold))
    ...
    private async Task<List<SearchItemDto>> SearchStadiumsWithFuzzyAsync(string searchTerm)
    {
        ...
        var fuzzyMatches = stadiums
            .Where(s => CalculateLevenshteinDistance(searchTerm, s.Name!.ToLower()) <= _fuzzySearchThreshold ||
                        (s.City != null && CalculateLevenshteinDistance(searchTerm, s.City.ToLower()) <=
                            _fuzzySearchThreshold))
    ...
    private async Task<List<Team>> SearchTeamsWithStrategyAsync(string query, SearchStrategy strategy, int limit)
    {
        return strategy switch
        {
            SearchStrategy.FullText when _enablePostgresFts => await SearchTeamsWithPostgreSqlAsync(query, limit),
    ...
    private async Task<List<Player>> SearchPlayersWithStrategyAsync(string query, SearchStrategy strategy, int limit)
    {
        return strategy switch
        {
            SearchStrategy.FullText when _enablePostgresFts => await SearchPlayersWithPostgreSqlAsync(query, limit),
    ...
    private async Task<List<Coach>> SearchCoachesWithStrategyAsync(string query, SearchStrategy strategy, int limit)
    {
        return strategy switch
        {
            SearchStrategy.FullText when _enablePostgresFts => await SearchCoachesWithPostgreSqlAsync(query, limit),
    ...
    private async Task<List<Stadium>> SearchStadiumsWithStrategyAsync(string query, SearchStrategy strategy, int limit)
    {
    ...
    private async Task<List<Season>> SearchSeasonsWithStrategyAsync(string query, SearchStrategy strategy, int limit)
    {
        var normalizedQuery = query.ToLower().Trim();
        using var scope = serviceScopeFactory.CreateScope();
        var context = scope.ServiceProvider.GetRequiredService<FootballDbContext>();
    ...
    private async Task<List<Player>> SearchPlayersWithFuzzyStrategyAsync(string query, int limit)
    {
        ...
        return players
            .Where(p => (p.FullName != null &&
                         CalculateLevenshteinDistance(searchTerm, p.FullName.ToLower()) <= _fuzzySearchThreshold) ||
                        (p.KnownName != null && CalculateLevenshteinDistance(searchTerm, p.KnownName.ToLower()) <=
                            _fuzzySearchThreshold))
            .Take(limit)
            .ToList();
    ...
    private async Task<List<Team>> SearchTeamsWithHybridStrategyAsync(string query, int limit)
    {
        ...
        var combined = ftResults.Concat(fuzzyResults)
            .GroupBy(t => t.Id)
            .Select(g => g.First())
            .Take(limit)
            .ToList();
    ...
    private async Task<List<SearchSuggestionDto>> GetTeamSuggestionsAsync(string normalizedQuery, int limit)
    {
        using var scope = serviceScopeFactory.CreateScope();
        var context = scope.ServiceProvider.GetRequiredService<FootballDbContext>();
        var teams = await context.Teams
            .Where(t => t.Name!.ToLower().Contains(normalizedQuery.ToLower()))
            .Take(limit)
            .AsNoTracking()
            .ToListAsync();

        return teams.Select(t => new SearchSuggestionDto
        {
            Text = t.Name ?? "",
            Type = "Team",
            Description = $"{t.City} - {t.League}",
    ...
    private async Task<List<SearchSuggestionDto>> GetPlayerSuggestionsAsync(string normalizedQuery, int limit)
    {
        using var scope = serviceScopeFactory.CreateScope();
        var context = scope.ServiceProvider.GetRequiredService<FootballDbContext>();
        var players = await context.Players
            .Where(p => p.FullName!.ToLower().Contains(normalizedQuery.ToLower()) ||
                        p.KnownName!.ToLower().Contains(normalizedQuery.ToLower()))
            .Take(limit)
            .AsNoTracking()
            .ToListAsync();

        return players.Select(p => new SearchSuggestionDto
        {
            Text = p.FullName ?? "",
            Type = "Player",
    ...
    private async Task<List<SearchSuggestionDto>> GetCoachSuggestionsAsync(string normalizedQuery, int limit)
    {
        using var scope = serviceScopeFactory.CreateScope();
        var context = scope.ServiceProvider.GetRequiredService<FootballDbContext>();
        var coaches = await context.Coaches
            .Where(c => c.FirstName!.ToLower().Contains(normalizedQuery.ToLower()) ||
                        c.LastName!.ToLower().Contains(normalizedQuery.ToLower()))
            .Take(limit)
            .AsNoTracking()
            .ToListAsync();

        return coaches.Select(c => new SearchSuggestionDto
        {
    ...
    private async Task<List<SearchSuggestionDto>> GetStadiumSuggestionsAsync(string normalizedQuery, int limit)
    {
        using var scope = serviceScopeFactory.CreateScope();
        var context = scope.ServiceProvider.GetRequiredService<FootballDbContext>();
        var stadiums = await context.Stadiums
        ...
        return stadiums.Select(s => new SearchSuggestionDto
        {
            Text = s.Name ?? "",
            Type = "Stadium",
    ...
    private float CalculateRelevanceScore(string query, string text)
    {
        ...
        if (textLower.StartsWith(queryLower)) return 0.8f;

        // Contains query gets medium score
        if (textLower.Contains(queryLower)) return 0.6f;
