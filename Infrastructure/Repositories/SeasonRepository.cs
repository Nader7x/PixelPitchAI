...
public class SeasonRepository(FootballDbContext context)
    : Repository<Season>(context),
        ISeasonRepository
{
    ...
    {
        return await _context.Seasons.FirstOrDefaultAsync(s => s.Name.ToLower() == name.ToLower());
    }
    ...
    {
        return await _context
            .Seasons.Where(s => s.Country.ToLower() == country.ToLower())
            .OrderByDescending(s => s.CurrentRound)
            .ToListAsync();
    }
    ...
    {
        return await _context
            .Seasons.Where(s => s.LeagueName.ToLower() == leagueName.ToLower())
            .OrderByDescending(s => s.CurrentRound)
            .ToListAsync();
    }

    public async Task<IReadOnlyList<Season>> GetSeasonMatches(
        int homeTeamSeasonId,
        int awayTeamSeasonId
    )
    {
        return await _context
            .Seasons.Include(s => s.Matches)!
