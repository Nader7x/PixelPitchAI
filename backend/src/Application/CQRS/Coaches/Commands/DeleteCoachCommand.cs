...
{
    public async Task<DeleteCoachCommandResponse> Handle(
        DeleteCoachCommand request,
    ...
    )
    {
        try
        {
            ...
            var matchesAsHomeCoach = await unitOfWork.Matches.FindAsync(
                m => m.HomeCoachId == request.Id,
                cancellationToken
            );
            var matchesAsAwayCoach = await unitOfWork.Matches.FindAsync(
                m => m.AwayCoachId == request.Id,
                cancellationToken
            );
        ...
        {
            return new DeleteCoachCommandResponse { Succeeded = false, Error = ex.Message };
        }
    }
}
